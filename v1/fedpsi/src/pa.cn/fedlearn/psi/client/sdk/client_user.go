...
func (p *PartyClient) CreateTask(t types.TaskCreateRequest) (bool, error) {
	u := "apis/v1/task"
	url1 := fmt.Sprintf("%s://%s/%s", p.Scheme, p.Server, u)
...
func (p *PartyClient) GetTask(taskUID string) (*types.Task, error) {
	u := fmt.Sprintf("apis/v1/task?task_uuid=%s", taskUID)
	url1 := fmt.Sprintf("%s://%s/%s", p.Scheme, p.Server, u)
	headers := map[string]string{
		"Content-type": "application/json",
		"Accept":       "text/plain",
...
func (p *PartyClient) TaskIntersectResult(taskUID string) (string, error) {
	u := fmt.Sprintf("apis/v1/task/intersect?task_uuid=%s", taskUID)
	url1 := fmt.Sprintf("%s://%s/%s", p.Scheme, p.Server, u)
	headers := map[string]string{
		"Content-type": "application/json",
		"Accept":       "text/plain",
