...
func (m *TaskMgr) TaskCreate(ctx context.Context, r types.TaskCreateRequest) (err error) {
	...
	for _, d := range r.TaskDatasets {
		if d.PartyName != config.GetConfig().PartyName {
			remoteParty = d.PartyName
		}
	}
...
func (m *TaskMgr) TaskCreateV2(ctx context.Context, r types.TaskCreateRequestV2) (err error) {
	log.Debugln(ctx, "TaskMgr.TaskCreateV2 is called")
	if r.TaskUID == "" {
		r.TaskUID = utils.UUIDStr()
	}
	// Set the initiator as the Initiator Party
	...
	if err = m.createTaskV2(ctx, r); err != nil {
		return
	}
	...
	ok, err := remoteClient.CreatePartyTaskV2(ctx, r)
	if err != nil {
		return fmt.Errorf("Remote Error:%v", err)
	}
	if !ok {
...
func (m *TaskMgr) PartyTaskCreate(ctx context.Context, r types.TaskCreateRequest) (err error) {
	log.Debugln(ctx, "TaskMgr.PartyTaskCreate is called")
	return m.createTask(ctx, r)
}

func (m *TaskMgr) PartyTaskCreateV2(ctx context.Context, r types.TaskCreateRequestV2) (err error) {
	log.Debugln(ctx, "TaskMgr.PartyTaskCreateV2 is called")
	return m.createTaskV2(ctx, r)
}

func (m *TaskMgr) createTask(ctx context.Context, r types.TaskCreateRequest) (err error) {
	rv2 := toTaskCreateRequestV2(r)
	return m.createTaskV2(ctx, rv2)
}

func (m *TaskMgr) createTaskV2(ctx context.Context, r types.TaskCreateRequestV2) (err error) {
	...
	if t.Mode == model.TaskRunMode_Server {
		t.ServerIP, t.ServerPort = worker.GetAvailableIpAndPort()
	}
...
func (m *TaskMgr) TaskConfirm(ctx context.Context, r types.TaskConfirmRequest) (err error) {
	...
	r.PSIServerIP, r.PSIServerPort = worker.GetPublicIPAndPort()
...
func (m *TaskMgr) TaskList(ctx context.Context, r types.TaskListRequest) (taskList []*types.Task, err error) {
	...
	if r.JobUID != "" {
		listFunc = func() ([]*model.Task, error) {
			return model.ListTasksOfJob(r.JobUID)
		}
	}
...
func (m *TaskMgr) TaskIntersectRead(ctx context.Context, r types.TaskIntersectionDownloadRequest) (data []byte, err error) {
	...
	if task.Mode == model.TaskRunMode_Client {
		if intersect, err := worker.ResultInfo(ctx, r.TaskUID); err != nil {
			...
			intersectData = worker.PostProcessClientResult(intersect)
		}
	}
