...
func (m *PartyMgr) PartyUpdate(ctx context.Context, r *types.PartyUpdateRequest) error { // Only update token
	...
	p.Token = r.Token
	p.Scheme = r.Scheme
	p.ControllerServer = r.ControllerServer
	p.ControllerPort = r.ControllerPort
...
func (m *PartyMgr) PartyWorkerReady(ctx context.Context, name string) error {
	log.Debugln(ctx, "PartyMgr.PartyWorkerReady is called")
	remoteClient, err := getRemoteClient(name)
	if err != nil {
		log.Errorf(ctx, "Get party client for %s error:%v", name, err)
		return fmt.Errorf("Get party client for %s error:%w", name, err)
	}
	si, err := remoteClient.PartyWorkerStatus(ctx)
	if err != nil {
		log.Errorf(ctx, "Get party worker status for %s error:%v", name, err)
		return err
	}
	log.Infof(ctx, "Party worker %s is OK", si.PartyName)
	return err
}
