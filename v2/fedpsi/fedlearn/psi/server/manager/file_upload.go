...
func (mgr *UploaderManager) MergeChunk(ctx context.Context, r types.ChunkMergeRequest) error {
	...
	dataset.Md5 = md5Str
	lineCount, _, repeatedLine, err := utils.FileMetaInfo(targetPath)
	if err != nil {
		log.Errorf(ctx, "Error to get %s file lines:%v", targetPath, err)
		return err
	}
	if repeatedLine != "" {
		log.Errorf(ctx, "BAD FILE: The file contents line: %s is repeated", repeatedLine)
		return fmt.Errorf("上传的数据集有重复，请去重后重新上传，重复内容是: %s", repeatedLine)
	}
