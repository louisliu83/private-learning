...
// BatchJobSubmit submit a batch jobs to be scheduled.
// As the dataset of the job cannot exceeds max lines, JobScheduler will split the job to tasks accord the dataset count
func (m *TaskMgr) BatchJobSubmit(ctx context.Context, r types.BatchJobSubmitRequest) map[string]string {
	log.Debugln(ctx, "TaskMgr.BatchJobSubmit is called")
	errors := make(map[string]string)
	for _, j := range r {
		// Create Task in local db
		if j.JobUID == "" {
			j.JobUID = utils.UUIDStr()
		}

		// Set the initiator as the Initiator Party
		j.Initiator = config.GetConfig().PartyName
		// Set the initiator as the client mode
		j.Mode = model.TaskRunMode_Client

		j.Protocol = strings.ToUpper(j.Protocol)
		if j.Protocol != worker.DiffieHellman && j.Protocol != worker.OT {
			//return fmt.Errorf("task protocol must be [OT|DH], not %s", j.Protocol)
			errors[j.Name] = fmt.Sprintf("task protocol must be [OT|DH], not %s", j.Protocol)
			continue
		}

		if err := m.submitJob(ctx, j); err != nil {
			log.Errorf(ctx, "submitJobFailed:%v", err)
			errors[j.Name] = fmt.Sprintf("submitJobFailed:%v", err)
			continue
		}

		remoteParty := j.PartyDataset.PartyName
		remoteClient, err := getRemoteClient(remoteParty)
		if err != nil {
			log.Errorf(ctx, "Get party client for %s error:%v", remoteParty, err)
			errors[j.Name] = fmt.Sprintf("Get party client for %s error:%v", remoteParty, err)
			continue
		}
		j.Mode = GetPeeringMode(j.Mode)
		j.LocalDataset, j.PartyDataset = j.PartyDataset, j.LocalDataset
		ok, err := remoteClient.PartyJobSubmit(ctx, j)
		if err != nil || !ok {
			log.Errorf(ctx, "remote party submit job %s error:%v", j.Name, err)
			errors[j.Name] = fmt.Sprintf("submitJobFailed:%v", err)
			continue
		}
	}
...
func (m *TaskMgr) PartyJobSubmit(ctx context.Context, r types.JobSubmitRequest) (err error) {
	...
	err = m.submitJob(ctx, r)
	if err != nil {
		return
	}
	/***	if r.ActUID != "" {
		jobConfirmRequest := types.JobConfirmRequest{
			JobUID: r.JobUID,
		}
		err = m.JobConfirm(ctx, jobConfirmRequest)
	}***/
	return
}
...
func (m *TaskMgr) JobConfirm(ctx context.Context, r types.JobConfirmRequest) (err error) {
	...
	if job.Status != model.JobStatus_WaitingPartyConfirm {
		log.Errorf(ctx, "confirm job %s failed %v \n", r.JobUID, err)
		return fmt.Errorf("confirm this job failed %s", r.JobUID)
	}
