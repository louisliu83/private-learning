...
func (e *TaskExecutor) Start(ctx context.Context, task *model.Task) error {
	...
	go func() {
		...
		serverIP := "127.0.0.1"
		serverPort := portmgr.GetPortOfTask(task.Uuid)
		...
		if task.Mode == model.TaskRunMode_Client {
			stopClientProxyListenerAndReleasePort(task.Uuid)
		}
		if task.Mode == model.TaskRunMode_Server {
			releaseServerPort(task.Uuid)
		}
...
func stopClientProxyListenerAndReleasePort(taskUID string) error {
	grpcproxy.StopTCP2GrpcServer(taskUID)
	p := portmgr.GetPortOfTask(taskUID)
	if p > 0 {
		portmgr.PSIClientPortManager.ReleasePort(p)
	}
	return nil
}

func releaseServerPort(taskUID string) error {
	p := portmgr.GetPortOfTask(taskUID)
	if p > 0 {
		portmgr.PSIServerPortManager.ReleasePort(p)
	}
	return nil
}
