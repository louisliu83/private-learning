...
func ListTasksByPage(pageNum int, pageSize int) ([]*Task, int64, error) {
	o := orm.NewOrm()
	var objs []*Task
	count, _ := o.QueryTable("task").Count()
...
// ListTasksOfJob return all tasks of the given job
func ListTasksOfJobByPage(jobUID string, pageNum int, pageSize int) ([]*Task, int64, error) {
	o := orm.NewOrm()
	var objs []*Task
	count, _ := o.QueryTable("task").
		Filter("job_uid", jobUID).Count()
	_, err := o.QueryTable("task").
		Filter("job_uid", jobUID).
		Limit(pageSize).
		Offset((pageNum - 1) * pageSize).
